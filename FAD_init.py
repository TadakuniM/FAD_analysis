...
""" Material parameters """
sigma_y = 700#350 # Yielding Stress [MPa]
sigma_u = 800#500 # Ultimate Tensile Strength [MPa]
K_ic = 100#130 # Fracture toughness [MPa*m^0.5]
...
Kr_cutoff = np.array([0,(1+0.5*Lr_max**2)**(-1/2)*(0.3+0.7*np.exp(-0.6*Lr_max**2))])
...
Ki_prime,Ki_secondary = Y_ave*sigma_0*np.sqrt(np.pi*a_i), Y_ave*sigma_res*np.sqrt(np.pi*a_i)
...
Kr = fad_assess_line(Lr_assess,Kr_assess,K_ic,Ki_secondary,Lr)

""" Decide the crossing point """
idLr = np.argwhere(np.sign(np.round(Kr_diag - Kr, 4)) == 0) 
print(len(idLr))

""" Plot the results """
figure = plt.figure()
plt.plot(Lr_assess, Kr_assess, 'ms', ms=5, label='Assessment point', color='black')
plt.text(Lr_assess,Kr_assess,'({Lr},{Kr})'.format(Lr=np.round(Lr_assess,decimals=3),Kr=np.round(Kr_assess,decimals=3)), fontsize=10,ha='right',va='bottom')
plt.xlabel("Lr")
plt.ylabel("Kr")
...
if len(idLr)==0:
    ...
    plt.plot(Lr[idLr], Kr[idLr], 'ms', ms=5, label='Failure point', color='green')
    for i in idLr.ravel():
        plt.text(Lr[i], Kr[i], '({Lr}, {Kr})'.format(Lr=np.round (Lr[i],decimals = 3), Kr=np.round(Kr[i],decimals=3)), fontsize=10,ha = 'left')
    SoF_fad = +Lr[idLr]/Lr_assess
print(SoF_fad)
plt.plot(Lr, Kr_diag, color='red', label='FAD')
plt.plot(Lr_cutoff,Kr_cutoff, color='red',label='')
plt.plot(Lr, Kr, color='blue', label='')
plt.xlim(0,1.5); plt.ylim(0,1.2)
plt.legend()
plt.show()
figure.savefig("FAD_init_crack.png")
plt.close()